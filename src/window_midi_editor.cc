...
void drawGrid(int xoffset, int yoffset, int col_width,int row_width) {
    ...
    for (int i = 0; i < (w/col_width)+1; i++) {
        DrawLine(i*col_width+xfrac_off, 0, i*col_width+xfrac_off, h, BLACK);
    }
    for (int i = 0; i < (h/row_width)+1; i++) {
        DrawLine(0, i*row_width+yfrac_off, w, i*row_width+yfrac_off, BLACK);
    }
}

Rectangle getNoteRect(const Sheet& sheet, int i) {
   return Rectangle{sheet.timestamps_start[i]*pixels_per_second+xscroll_offset,
...
void moveTool(Vector2 mPos, Rectangle& nRec, Sheet& sheet, int i) {
    if(dragging_note == i) {
        ...
        DrawRectangleLines(nRec.x+1, nRec.y+1, nRec.width-1, nRec.height-1, ORANGE);
        DrawRectangleLines(nRec.x+2, nRec.y+2, nRec.width-2, nRec.height-2, ORANGE);
    }
    if(IsMouseButtonUp(MOUSE_LEFT_BUTTON)) dragging_note = -1;

    if(IsMouseButtonDown(MOUSE_LEFT_BUTTON) and dragging_note == -1 and CheckCollisionPointRec(mPos, nRec)) {
        dragging_note = i;
...
void splitTool(Vector2 mPos, Rectangle nRec, Sheet& sheet, int i) {
    if(CheckCollisionPointRec(mPos, nRec) and IsMouseButtonReleased(MOUSE_BUTTON_LEFT)) {
        ...
        sheet.attack_velocities.push_back(sheet.attack_velocities[i]);
        sheet.timestamps_start.push_back(sheet.timestamps_start[i] + duration_cut);
    }
}

void removeNote(Sheet& sheet, int to_remove) {
    ut::swap(sheet.attack_velocities[to_remove], sheet.attack_velocities[sheet.attack_velocities.size()-1]);
    sheet.attack_velocities.pop_back();
    ut::swap(sheet.pitch[to_remove], sheet.pitch[sheet.pitch.size()-1]);
    sheet.pitch.pop_back();
    ut::swap(sheet.durations[to_remove], sheet.durations[sheet.durations.size()-1]);
    sheet.durations.pop_back();
    ut::swap(sheet.timestamps_start[to_remove], sheet.timestamps_start[sheet.timestamps_start.size()-1]);
...
void toolCreate(Sheet& sheet, Vector2 mPos) {
    palette_sheet.timestamps_start[0] = (mPos.x-xscroll_offset)/pixels_per_second;
...
void drawCursor() {
    ...
    float cross_size = 16;
    DrawLineEx(mPos + Vector2{0, cross_size}, mPos - Vector2{0, cross_size}, 3, BLACK);
    DrawLineEx(mPos + Vector2{cross_size, 0}, mPos - Vector2{cross_size, 0}, 3, BLACK);
}
...
void drawSoundTimeline(Sheet& sheet) {
    ...
    ClearBackground(Color{100, 0, 100});
    if(xscroll_offset > 0)
        BeginScissorMode(xscroll_offset, 0, w, h);
    ClearBackground(Color{200, 0, 200});
    EndScissorMode();
    ...
    for (int i = 0; i < size; i++) {
        ...
        BeginScissorMode(nRec.x, nRec.y+row_width-(sheet.attack_velocities[i]/128.)*row_width, nRec.width, nRec.height);
        DrawRectangleRounded(nRec, 0.5, 5, DARKGRAY);
        EndScissorMode();
        if(tool == Move) moveTool(mPos, nRec, sheet, i);
        else if(tool == Split) {
